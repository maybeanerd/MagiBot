...
module.exports = {
  main: async function main(bot, msg) {
    ...
    switch (command) {
    case 'ban':
      uid = await cmds.findMember(msg.guild, mention);
      if (mention && uid) {
    ...
    case 'admin':
      ...
      if (mention && rid) {
        ...
        } else if (await cmds.yesOrNo(msg, `Do you want to remove <@&${rid}> from the admin roles?`, `Cancelled removing <@&${rid}> from the admin roles`)) {
    ...
    case 'notification':
      ...
      } else if (await cmds.yesOrNo(msg, `Do you want to deactivate MagiBot notifications in <#${await msg.channel.id}>?`, `Cancelled deactivating notifications in <#${await msg.channel.id}>`)) {
    ...
    case 'prefix':
      if (mention) {
        if (await cmds.yesOrNo(msg, `Do you want to change the prefix in ${msg.guild.name} from \`${bot.PREFIXES[msg.guild.id]}.\` to \`${mention}.\` ?`, 'Cancelled changing the prefix.')) {
          const newpref = await data.setPrefixE(msg.guild.id, mention, bot);
          if (newpref) {
